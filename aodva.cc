...
void AODV::recvRequest(Packet *p)
{
  ...
  iEnergy=  iNode->energy_model()->energy();
    ...
    FILE *fp;
    fp = fopen("test.txt", "a");
    fprintf(fp, "\n%.6f  %s: di node %d dengan energy %.4f, packet dari source node %d menuju node destination %d dengan rq_lifetime %.4f",  CURRENT_TIME, __FUNCTION__, index, iEnergy, rq->rq_src, rq->rq_dst, rq->rq_min_life);
    fclose(fp);   
   #endif // DEBUG
  ...
  if (rq->rq_min_life > iEnergy && iEnergy > 0){
      ...
      FILE *fp;
      fp = fopen("test.txt", "a");
      fprintf(fp, "\n%.6f  update lifetime dari %.4f menjadi %.4f", CURRENT_TIME, rq->rq_min_life, iEnergy);
      fclose(fp);   
     #endif // DEBUG
...
void AODV::forward(aodv_rt_entry *rt, Packet *p, double delay)
{
  ...
  struct hdr_ip *ih = HDR_IP(p);
    ...
    iEnergy=  iNode->energy_model()->energy();
    fp = fopen("test.txt", "a");
    fprintf(fp, "\n%.6f  Position dari node %d is X =%.4f and Y =%.4f",CURRENT_TIME, index, xpos, ypos); 
    fprintf(fp, "\n%.6f  Energi update dari node %d tersisa %.4f",CURRENT_TIME, index, iEnergy); 
    fclose(fp);
